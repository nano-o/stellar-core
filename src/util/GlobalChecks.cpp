...
maybeThrowOrAbort(std::string message)
{
    if (gAssertThrowMode)
    {
...
dbgAbortImpl(char const* file, int line)
{
    ...
    maybeThrowOrAbort(std::string("dbgAbort at ") + file + ":" +
                      std::to_string(line));
}
...
printErrorAndAbort(char const* s1, char const* s2)
{
    ...
    std::fflush(stderr);
    printCurrentBacktrace();
    maybeThrowOrAbort(std::string(s1) + s2);
}
...
printAssertFailureAndAbort(char const* s1, char const* file, int line)
{
    ...
    std::fflush(stderr);
    printCurrentBacktrace();
    maybeThrowOrAbort(std::string(s1) + " at " + file + ":" +
                      std::to_string(line));
}
...
printAssertFailureAndThrow(char const* s1, char const* file, int line)
{
    ...
    std::fflush(stderr);
    printCurrentBacktrace();
    throw std::runtime_error(std::string(s1) + " at " + file + ":" +
                             std::to_string(line));
}
}
